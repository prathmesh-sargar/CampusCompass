            ...
            <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-8">
                {features.map((feature, index) => (
                    <Link key={index} to={feature.link}>
                        <div className="flex flex-col items-center bg-black text-white p-6 rounded-xl shadow-lg dark:shadow-gray-800 transition-transform transform hover:scale-105 cursor-pointer">
                            <img 
                                alt={feature.title}
                                loading="lazy"
                                width={feature.imgWidth}
                                decoding="async"
                                className=" bg-white w-16 h-16 mb-4 rounded-full"
                                src={feature.imgSrc}
                            />
