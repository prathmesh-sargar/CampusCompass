...
const HomePage = () => {
  ...
  useEffect(() => {
    // 🛑 Prevent double-init (React StrictMode safety)
    if (initializedRef.current) return;
    initializedRef.current = true;

    // 🛑 Guard WebGL support
    if (!window.WebGLRenderingContext || !canvasRef.current) {
    ...
    return () => {
      ...
      renderer.dispose();
      renderer.forceContextLoss(); // 🔥 prevents future WebGL crashes
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-16">
                <div className="lg:w-1/2 text-center lg:text-left">
                  ...
                  <p className="text-xl text-gray-300 mb-10">
                    Track progress, analyze resumes, practice interviews, and grow smarter.
                  </p>

                  <Link
                    to="/profile"
                    className="px-8 py-4 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 shadow-lg"
                  >
                    Start Tracking
                  </Link>
                </div>
